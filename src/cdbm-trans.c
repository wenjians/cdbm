...
T_cdbm_trans_id cdbm_create_transaction(char* trans_name)
{
    ...
    AAT_STD_ASSERT(cdbm_trans->plain_config != NULL);

    cdbm_trans->xml_config = gfi_print_buf_creat(trans_name, AAT_MOD_CDBM,
                                  GFI_PRITN_BUF_FLAG_OVERSTOP, CDBM_CONFIG_MAX_SIZE);
    AAT_STD_ASSERT(cdbm_trans->xml_config != NULL);

    /* add to transaction session list */
    gfi_list_init(&cdbm_trans->list_node);
    gfi_lock(&g_cdbm_db.trans_list_lock);
    gfi_list_insert_last(g_cdbm_db.trans_list, &cdbm_trans->list_node);
...
T_global_rc cdbm_close_transaction(T_cdbm_trans_id cdbm_trans)
{
    ...
    if (!gfi_list_is_entry_in_list(g_cdbm_db.trans_list, &cdbm_trans->list_node))
    {
        CDBM_LOG(CDBM_EVT_ERROR, "cdbm close transaction which is not exist, caller (0x%x)", ret_addr);
        return RC_CDBM_TRANS_NOT_IN_LIST;
    }

    gfi_print_buf_destroy(cdbm_trans->plain_config);
    gfi_print_buf_destroy(cdbm_trans->xml_config);

    /* remove from transaction session list */
    gfi_lock(&g_cdbm_db.trans_list_lock);
    gfi_list_remove(g_cdbm_db.trans_list, &cdbm_trans->list_node);
    gfi_unlock(&g_cdbm_db.trans_list_lock);
